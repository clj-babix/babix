...
- **In-place materialization with stable identifiers before hashing**: A full static walk over the Derivation Description is used to discover which references are actually used (including inside strings and builder-private data), so that unused inputs can be pruned. At locking time, only the `:inputs` section is replaced in-place with stable identifiers (shape `{:ref <keyword> :id <store-identity>}` or equivalent). References that appear elsewhere in the description — in categorized input lists, as the value of `:builder` or `:activator`, inside strings, or in any other body data — remain as keywords that point into the now-pinned `:inputs` map. The locked form is fully explicit and self-contained for the inputs that are actually referenced. See the "Stable Identifier" entry in [docs/CONTEXT.md](docs/CONTEXT.md) for the authoritative rule.
